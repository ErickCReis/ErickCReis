...
At some point this portfolio stopped being just a static page. I kept adding telemetry, API routes, cursor presence, background polling, local data, and internal jobs. It was still a small project, but its deployment shape had started to matter.

This rewrite was not about escaping Cloudflare or saving money. I wanted to make one machine the unit of deployment. [Astro](https://astro.build/) would still produce static pages and [Elysia](https://elysiajs.com/) would still handle live routes, but the whole application would ship as one container to an unused laptop.
...
Astro builds the mostly static site. [Solid](https://www.solidjs.com/) hydrates only the live islands. Elysia owns the API, SSE, WebSocket, and internal routes. [Bun](https://bun.sh/) runs the scripts and compiles the server.
...
The application uses `DATA_DIR=/app/data` for SQLite and local files, and Docker Compose mounts `./data` from the host into that directory. The container can be rebuilt without taking its state with it. Compose also gives the process a restart policy and explicit CPU and memory limits.
...
Self-hosting made this application simpler to deploy, not simpler to operate. I traded managed runtime boundaries for responsibilities I can see: one process, one data directory, one backup problem, and one real-time layer I still need to harden.
