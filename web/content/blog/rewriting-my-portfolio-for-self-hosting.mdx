...
I rewrote it because I wanted the deployment to have a physical answer: one machine. [Astro](https://astro.build/) would still produce static pages and [Elysia](https://elysiajs.com/) would still handle live routes, but the whole application would ship as one container to an unused laptop.
...
I still like Cloudflare, and it remains part of this setup. DNS and connectivity stay at the edge; the compute and persistent data moved to the laptop. Here, self-hosting describes where the application runs while external services continue to handle other parts.
...
I chose Dokploy because it works directly with [Docker](https://www.docker.com/) and keeps the deployment workflow easy to inspect. The infrastructure still has several pieces, but the application has fewer runtime boundaries: one deploy target, one container, and one server process.

Those paths meet at the running container, but they carry different things. A new deploy replaces the container while the mounted `/app/data` directory remains.
...
An explicit data directory makes the state easy to locate and snapshot. I still need an automated off-machine copy. The laptop also makes power, storage, and network failures my responsibility instead of the provider's.
...
Self-hosting made this application simpler to deploy and gave me more operational responsibility. The smaller shape is easier to see; its safety is still my responsibility. I traded managed runtime boundaries for one process, one data directory, one backup problem, and one real-time layer I still need to harden.
