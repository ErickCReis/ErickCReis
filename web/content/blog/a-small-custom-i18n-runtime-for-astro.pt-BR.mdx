---
title: "Um pequeno runtime customizado de i18n para o Astro"
...
date: "2026-08-29"
tags: [i18n, astro, vite]
---

import { TranslationCatalogLab } from "@web/features/blog-series/labs/translation-catalog-lab";

A [camada do blog](/pt-BR/content/the-blog-layer-static-pages-live-view-counts) trata os artigos em inglês e português brasileiro como arquivos MDX separados. Essa é a fronteira certa para textos longos: tradução é um trabalho editorial, não uma consulta em um catálogo de strings.
...
Criei uma integração customizada para esse requisito mais estreito. Ela não é um framework genérico de internacionalização. A integração coleta strings de origem estáticas, gera catálogos TypeScript indexados por hashes, expõe um módulo virtual e volta ao texto em inglês sempre que uma tradução está ausente.
...
As duas funções vêm de `virtual:translate`, junto com `locales`, `defaultLocale`, `resolveLocale`, `getPathLocale` e `getLocale`. A integração injeta uma declaração para esse módulo virtual, incluindo uma união `Locale` derivada da lista de idiomas configurada e overloads para as duas formas de `t()`.
...
<TranslationCatalogLab client:visible locale="pt-BR" />
...
Esse runtime não oferece mensagens ICU, negociação de idioma, regras gramaticais de plural, interpolação de rich text, serviços de tradução nem um fluxo de gerenciamento de conteúdo. O site ainda usa `Intl` diretamente para datas e números, e posts completos continuam em arquivos próprios.

São essas ausências que mantêm a ferramenta customizada compreensível. Para dois idiomas conhecidos e algumas dezenas de strings estáticas da interface, um hook do Astro, um plugin do Vite, dois catálogos gerados e um pequeno runtime resolvem o problema real. Um produto mais amplo cruzaria o ponto em que manter todas essas capacidades ausentes custa mais do que adotar um sistema de i18n estabelecido.
