...
```mermaid center
flowchart TB
  Source["Static t() calls"] --> AST["Analyze ESTree"]
  AST --> Collector["Deduplicate source strings"]
  Collector --> Catalogs["Generated locale catalogs"]
  Catalogs -->|"next build"| Virtual["virtual:translate"]
  Virtual --> SSR(["Static pages<br/>from Astro"])
  Virtual --> Client(["Hydrated<br/>Solid islands"])
```
