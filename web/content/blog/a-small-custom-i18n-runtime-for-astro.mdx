...
The [blog layer](/blog/the-blog-layer-static-pages-live-view-counts) treats English and Brazilian Portuguese articles as separate MDX files. That is the right boundary for long-form writing, where translation requires editorial work beyond a string lookup.
...
I built a custom integration for that narrower requirement. It collects static source strings, generates hash-keyed TypeScript catalogs, exposes a virtual module, and falls back to the English source whenever a translation is absent.
...
The runtime's scope excludes ICU messages, locale negotiation, grammatical plural rules, rich-text interpolation, translation services, and a content-management workflow. The site still uses `Intl` directly for dates and numbers, and whole blog posts still have dedicated files.
