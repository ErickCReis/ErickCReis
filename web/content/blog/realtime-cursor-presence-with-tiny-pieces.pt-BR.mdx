---
title: "Presença de cursores em tempo real com poucas peças"
...
O [post anterior](/pt-BR/blog/astro-static-shell-solid-live-layer) terminou na fronteira da ilha do Solid. Este acompanha um ponteiro atravessando essa fronteira.

Ao mover o ponteiro na página inicial, um pequeno marcador o acompanha. Se a página for aberta em outro navegador, esse marcador aparece ali como um cursor remoto. Gosto do resultado visível porque ele é divertido; gosto da implementação porque ela continua enxuta. É um bom exemplo da arquitetura por trás do site: poucas peças, cada uma responsável por uma ponta da interação.
...
Resisti a adicionar mensagens para entrada, saída ou salas. O site tem uma única superfície pública de cursores, e uma atualização de posição é tudo de que ela precisa. O protocolo reduzido também deixa claro o seu limite. Seu escopo termina na presença transitória.
...
Por isso, as [rotas em tempo real](https://github.com/ErickCReis/ErickCReis/blob/main/server/live/routes.ts) atribuem ao WebSocket um ID em um cookie `HttpOnly` durante o upgrade. O cookie usa `SameSite=Strict`, é seguro em produção e não pode ser acessado pelo JavaScript do cliente. Um pequeno endpoint `GET /live/id` retorna o ID existente ou, quando o cookie está ausente, cria um ID, armazena-o no mesmo cookie `HttpOnly` e então o retorna. Assim, a camada de renderização descobre o próprio ID sem tornar o cookie legível.
...
Posições e cores ainda são entradas públicas não confiáveis, então essa verificação não forma um modelo de segurança completo. Ela preserva a propriedade da qual a funcionalidade depende: uma conexão não pode publicar como o cursor de outra.
...
O hook amostra o movimento para a tela e não garante a entrega. Se o socket não estiver aberto, a atualização atual é descartada. Em pouco tempo, uma posição mais nova tomará seu lugar.
...
<CursorProtocolLab client:load locale="pt-BR" />
...
```mermaid center
stateDiagram-v2
  [*] --> Connected: primeiro acesso
  Connected --> Reconnecting: conexão perdida
  Reconnecting --> Connected: nova tentativa em 1 segundo
  Reconnecting --> [*]: sem assinantes
  Connected --> [*]: último assinante
```
...
Essas pequenas peças dão vida a uma página que continua estática sem transformar posições de cursores em dados duráveis.

O próximo post reutilizará essa preferência por soluções pequenas com dados que mudam mais devagar: a reprodução do Spotify e as contribuições do GitHub coletadas pelo servidor e apresentadas como telemetria pessoal.
