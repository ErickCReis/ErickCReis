---
title: "A camada do blog: páginas estáticas, contagens dinâmicas"
...
import { ViewCounterLab } from "@web/features/blog-series/labs/view-counter-lab";
...
flowchart LR
  ...
  SQLite["Totais no SQLite"] --> ReadIsland["Hidratador de contagens da listagem"]
...
A página do artigo usa outra ilha, [`PostViewCounter`](https://github.com/ErickCReis/ErickCReis/blob/main/web/islands/post-view-counter.tsx). Ao ser montada, ela envia `POST /content/views` com a chave daquele artigo e exibe o total retornado. Se o documento for aberto em uma aba em segundo plano, a ilha espera até que ele fique visível. O preload de um navegador não deve parecer uma pessoa lendo um post.

```mermaid center
sequenceDiagram
  participant Page as Artigo estático
  participant Island as Contador do Solid
  participant API as Rota de conteúdo do Elysia
  participant DB as SQLite

  Page->>Island: montagem
  Island->>Island: aguarda documento visível
  Island->>API: POST da chave do artigo
  API->>DB: deduplica e incrementa
  DB-->>API: total + indicador counted
  API-->>Island: total atual
  Island-->>Page: substitui o placeholder
```
...
O laboratório abaixo torna essa transação visível. Leia como o navegador A duas vezes: a primeira requisição incrementa o total, enquanto a segunda encontra o token ativo e não o altera. O navegador B tem um token independente. Você também pode ocultar a aba para reter uma leitura no controle de visibilidade ou avançar o relógio em 25 horas e tornar um navegador elegível novamente. Note que a metade MDX nunca muda; apenas o número de runtime e o conjunto de tokens temporários se movimentam.

<ViewCounterLab client:visible locale="pt-BR" />

As tabelas de visualizações não armazenam endereço IP, user agent, conta nem referer. Elas armazenam o mesmo ID opaco de visitante ao lado de cada artigo visto recentemente, o que permite correlacionar as visualizações durante a janela de deduplicação. Registros vencidos são removidos em uma gravação posterior, não por uma tarefa exclusiva de limpeza. Este é um pequeno contador, não analytics anônimo, e essa distinção faz parte da sua descrição de privacidade.
