...
The [Spotify and GitHub panels](/blog/personal-telemetry-from-spotify-and-github) have different credentials, polling intervals, cache policies, and empty states. Their components do not know any of that.

That is the job of the stats pipeline.

The hard part was letting every source keep its own clock while giving the browser one place to bootstrap, listen, and decode. I wanted the panels to feel coordinated without making their collectors pretend to work at the same pace.
...
Each module still owns its state, retention limit, persistence choices, and source-specific errors. The interface only defines what the delivery layer needs.
...
`GET /stats/history` returns both forms for all modules and uses `Cache-Control: no-store`. The response bootstraps process state, so an intermediary cache cannot decide its freshness.
...
The next post moves below the API routes and explains how the same Bun process also serves Astro's static output as the production site.
