...
```mermaid center
flowchart TB
  Runtime["Runtime sources<br/>OS · uptime · presence"] --> Collectors["Independent stat modules"]
  Services["Service sources<br/>Spotify · GitHub · disk"] --> Collectors
  Collectors --> History["GET<br/>/stats/history"]
  Collectors --> Gate["Per-connection<br/>version gate"]
  Gate --> SSE["GET /stats/stream<br/>SSE"]
  History --> Codec["Shared<br/>tuple codecs"]
  SSE --> Codec
  Codec --> Stores["Solid stores<br/>latest + history"]
