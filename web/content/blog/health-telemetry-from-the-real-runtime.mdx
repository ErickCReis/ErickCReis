...
The [previous post](/blog/shipping-astro-from-a-bun-server) put Astro's files and the Bun server inside one container. Once that container became the production unit, a simple question stopped having a simple answer: when the site displays “system health,” which system is it describing?
...
When I first put these readings beside one another, their proximity made them look interchangeable. The telemetry on this site keeps four observer lenses (process, cgroup, host, and outside) separate because one measurement cannot describe the whole deployment.

```mermaid center
flowchart LR
  Process["Bun<br/>CPU"] --> Panels["Health panels"]
  Cgroup["Cgroup<br/>CPU + memory"] --> Panels
  Host["Laptop<br/>battery"] --> Panels
  Outside["UptimeRobot<br/>availability"] --> Panels
```
...
The result is the share of the deployment's CPU capacity used by this process, clamped between zero and 100 percent. Docker Compose currently limits the service to one CPU, so the panel shows how much of the CPU assigned to this application its Bun process is consuming.
...
`memory.current` measures more than the Bun heap. It accounts memory to the cgroup boundary, which can include the process, native allocations, and filesystem cache. The value shows whether the container is approaching its enforced budget; it cannot diagnose which allocation inside Bun consumed it.
...
If the mount, battery, or files do not exist, the reader returns `null` values and the panel displays `n/a`. Host battery telemetry is optional and specific to deployments that expose it.
...
<RuntimeVantageLab client:load locale="en-US" />

The gauge needs context. Its observer, denominator, and blind spot are part of the value; without those labels, a precise percentage can still tell the wrong story.
...
A containerized application exposes several systems to measure. This site measures process work against its cgroup allowance, opts into one host hardware signal, and asks an external observer whether the full public route works. Keeping those perspectives separate prevents one number from being mistaken for the state of the whole deployment.
