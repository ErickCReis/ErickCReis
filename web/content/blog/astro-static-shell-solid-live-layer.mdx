...
In the [previous post](/blog/rewriting-my-portfolio-for-self-hosting), I compressed the frontend architecture into one sentence: [Astro](https://astro.build/) turns most of the site into HTML, and [Solid](https://www.solidjs.com/) starts only the live islands in the browser.
...
Astro's [`client:only`](https://docs.astro.build/en/reference/directives-reference/#clientonly) directive skips rendering the component into HTML during the build and renders it in the browser instead. With no server-rendered component markup to resume, the browser performs a client render. The directive defines where rendering happens and carries no automatic performance benefit.
...
The empty state is valid. Until the island starts, there are no floating telemetry panels or remote cursors. The heading, project links, navigation, and metadata do not need a loading placeholder because they were never inside the island.
...
In this project, a useful island draws one boundary around behavior that needs to react together. Component count and the page's full framework tree do not define that boundary.

## JavaScript still exists outside Solid

The base layout still uses Astro's client router, and small page behaviors can remain ordinary scripts. The boundary describes ownership; JavaScript can exist on both sides.
