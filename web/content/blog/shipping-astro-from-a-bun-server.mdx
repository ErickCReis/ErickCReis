...
The public site has one front door, but two very different kinds of work happen behind it. The [stats pipeline](/content/the-stats-pipeline) ends at live Elysia routes; every article, stylesheet, font, image, and script is already finished before the server starts.

I wanted that single entry point without turning Astro into a server-rendered application. The [first post in this series](/content/rewriting-my-portfolio-for-self-hosting) showed the outer deployment shape—an Astro build, a compiled Bun server, and a small runtime image—but intentionally skipped how those pieces meet.
