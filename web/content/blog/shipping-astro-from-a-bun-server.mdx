...
I keep two separate build products:
...
The generator puts candidates in a map and throws as soon as a route already exists, including both file entries in the error. Sorted traversal makes diagnostics stable. An ambiguous static route fails during the build, before production needs a policy for resolving it.
...
That failure keeps the invalid state from becoming a rule that production has to resolve.
...
Astro still generates the pages ahead of time; Elysia does not render them. A request for an article reads the HTML Astro already generated. A request for `/stats/stream`, `/live`, or a content counter reaches runtime code in the same process.
