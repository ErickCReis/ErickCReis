...
`createBlogReport` cuida das regras de calendário e agregação. `renderBlogReport` cuida da apresentação. A tarefa agendada escolhe o horário. [`sendEmail`](https://github.com/ErickCReis/ErickCReis/blob/main/server/lib/email.ts) chama o provedor e lê o destinatário compartilhado de `EMAIL_TO`.

O adaptador de entrega recebe `subject`, `text` e `html`. Ele adiciona o remetente fixo e o destinatário de `EMAIL_TO` antes de chamar `resend.emails.send`. Se `RESEND_API_KEY` ou `EMAIL_TO` não existir, ele retorna `false`. Também retorna `false` para um erro do provedor ou uma exceção, depois de registrar o erro, e retorna `true` apenas quando a chamada à API termina sem erro.
