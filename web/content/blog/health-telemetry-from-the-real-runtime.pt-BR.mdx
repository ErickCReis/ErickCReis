---
title: "Telemetria de saúde a partir do runtime real"
...
Quando coloquei essas leituras lado a lado pela primeira vez, a proximidade fez com que parecessem intercambiáveis. Não são. A telemetria deste site mantém separadas quatro lentes de observação — processo, cgroup, host e observador externo — em vez de fingir que uma única medida consegue descrever todo o deploy.

```mermaid center
flowchart LR
  Process["Processo Bun"] --> CPU["Uso de CPU do processo"]
  Cgroup["Cgroup do Linux"] --> Limits["Cota de CPU + uso e limite de memória"]
  Host["power_supply do notebook"] --> Battery["Estado da bateria"]
  Outside["UptimeRobot"] --> Availability["Disponibilidade pública"]
  CPU --> Panels["Painéis de telemetria"]
  Limits --> Panels
  Battery --> Panels
  Availability --> Panels
```

## A CPU é relativa à capacidade do processo

O arquivo [`server/stats/system.ts`](https://github.com/ErickCReis/ErickCReis/blob/main/server/stats/system.ts) coleta uma amostra a cada 1,5 segundos. Para a CPU, ele começa com `process.cpuUsage()`, que informa o tempo de CPU gasto pelo processo Bun em espaço de usuário e de sistema. O módulo compara esses valores cumulativos com a amostra anterior e divide o trabalho pelo tempo real transcorrido.

O denominador também inclui a quantidade de CPUs disponível para o deploy. No cgroup v2, o módulo lê `cpu.max` e divide a cota pelo período. Ele aceita os arquivos equivalentes do cgroup v1 e usa `os.cpus().length` quando não há uma cota finita disponível.

```ts
const percent = (usedMicroseconds / (elapsedMicroseconds * CPU_COUNT)) * 100;
```

O resultado, portanto, não significa “quanto todos os processos do notebook estão ocupados?”. Ele representa a parcela da capacidade de CPU do deploy consumida por este processo, limitada entre zero e 100 por cento. Hoje, o Docker Compose limita o serviço a uma CPU, então o painel responde à pergunta operacionalmente útil: quanto da CPU atribuída a esta aplicação está sendo consumido pelo processo Bun?

A contagem de CPUs calculada a partir do cgroup arredonda a cota para um número inteiro e nunca fica abaixo de um. Isso é suficiente para o limite atual de uma CPU, mas seria um modelo aproximado para cotas fracionárias. A telemetria só é útil quando sua interpretação e suas limitações permanecem visíveis.
...
Esse número também não é o heap do Bun. `memory.current` contabiliza memória na fronteira do cgroup, o que pode incluir o processo, alocações nativas e cache do sistema de arquivos. É o sinal certo para “este container está se aproximando do orçamento imposto?”, não para diagnosticar qual alocação dentro do Bun o consumiu.
...
## A bateria é um vazamento intencional do host
...
Se a montagem, a bateria ou os arquivos não existirem, o leitor retorna valores `null`, e o painel mostra `n/a`. A telemetria da bateria do host é uma capacidade opcional, não uma suposição que todo deploy precisa atender.

Esse é um vazamento intencional e estreito pela fronteira do container. Montar todo o sistema de arquivos do host facilitaria a criação do painel e tornaria a aplicação muito mais difícil de confiar. Expor apenas o diretório de alimentação declara exatamente qual detalhe do host a aplicação tem permissão para observar.

## O uptime precisa de um ponto de vista externo

O valor original de uptime era próximo de um contador do processo. Ele pode dizer há quanto tempo o processo atual está vivo, mas não informa se DNS, túnel da Cloudflare, rede, container e servidor HTTP formaram um caminho público funcional.
...
- sequência atual de disponibilidade, calculada a partir do último log de recuperação.

Os dias anteriores à criação do monitor permanecem como `null` em vez de se tornarem períodos com zero por cento de disponibilidade. Entre dois retratos do servidor, o painel do Solid avança localmente uma sequência bem-sucedida a cada segundo, fazendo o contador visível se mover sem consultar a API externa a cada segundo.
...
<RuntimeVantageLab client:visible locale="pt-BR" />

O medidor não basta sozinho de propósito. Seu observador, denominador e ponto cego fazem parte do valor; sem esses rótulos, um percentual preciso ainda pode contar a história errada.
...
O painel e o alerta respondem a perguntas diferentes. O painel ajuda um visitante — ou eu — a entender o deploy. O e-mail pede uma ação quando uma condição específica do host cruza um limite. Os dois reutilizam o mesmo leitor da bateria, mas não devem compartilhar a mesma política de entrega.

Não existe um único “sistema real” por trás de uma aplicação em container. Este site mede o trabalho do processo em relação à capacidade do cgroup, escolhe observar um sinal do hardware do host e pergunta a um observador externo se o caminho público completo funciona. Mantenho essas perspectivas separadas porque a fronteira ao redor de um número é o que torna esse número honesto.
