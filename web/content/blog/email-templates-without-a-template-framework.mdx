...
`createBlogReport` handles calendar and aggregation rules. `renderBlogReport` handles presentation. The cron task selects the time. [`sendEmail`](https://github.com/ErickCReis/ErickCReis/blob/main/server/lib/email.ts) calls the provider and reads the shared recipient from `EMAIL_TO`.

The delivery adapter accepts `subject`, `text`, and `html`. It adds the fixed sender and the `EMAIL_TO` recipient before calling `resend.emails.send`. If `RESEND_API_KEY` or `EMAIL_TO` is missing, it returns `false`. It also returns `false` for a provider error or thrown exception after logging the error, and `true` only when the API call succeeds.
