...
```mermaid center
flowchart TB
  Raw["Provider record"] --> Adapter{"Provider adapter"}
  Adapter -->|Codex| Delta["Cumulative total → delta"]
  Adapter -->|Pi| Message["Assistant message usage"]
  Adapter -->|Claude| Cache["Input + cache normalization · message dedupe"]
  Delta --> Day["Normalized day"]
  Message --> Day
  Cache --> Day
```
...
The lab starts with a stale `codex/laptop` window and a fresh `claude/laptop` window. Keep the selected key to preview a replacement, or switch `sourceId` to `server` to preview an additive merge. Changing the generated age also reveals the gap between per-source freshness and the single aggregate flag.
