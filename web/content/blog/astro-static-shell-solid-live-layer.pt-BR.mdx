...
No [post anterior](/pt-BR/content/rewriting-my-portfolio-for-self-hosting), resumi a arquitetura do frontend em uma frase: o [Astro](https://astro.build/) gera a maior parte do site como conteúdo estático, e o [Solid](https://www.solidjs.com/) hidrata apenas as ilhas interativas.
...
```mermaid center
flowchart TB
  Build["Build do Astro"] --> Document["HTML, metadados, rotas e projetos"]
  Document --> Page["Página do portfólio"]

  Browser["Navegador inicia"] --> Island["Camada interativa do Solid"]
  Island --> Telemetry["Painéis de telemetria"]
  Island --> Cursors["Presença de cursores"]
  Telemetry --> Page
  Cursors --> Page
```
...
A diretiva [`client:only`](https://docs.astro.build/en/reference/directives-reference/#clientonly) do Astro não renderiza o HTML do componente durante o build e deixa essa renderização para o navegador. Ela não é apenas uma forma curta de dizer “torne isto interativo”, nem representa automaticamente uma otimização de desempenho.
