...
description: How two external APIs become small live panels with different polling, caching, and failure policies.
date: "2026-07-25"
tags: [telemetry, spotify, github]
---

import { SourcePolicyLab } from "@web/features/blog-series/labs/source-policy-lab";

The [cursor presence feature](/content/realtime-cursor-presence-with-tiny-pieces) moves short-lived data between browsers. The next two live surfaces move in the other direction: they bring outside context into the portfolio.

One panel shows what I am listening to on [Spotify](https://developer.spotify.com/documentation/web-api). Another summarizes my recent activity from the [GitHub GraphQL API](https://docs.github.com/en/graphql). Together they make the homepage say something about the present instead of only listing work from the past.

They look like neighboring pieces of telemetry, but their sources behave differently. Playback changes in seconds and can be absent. Contribution data changes slowly and is useful after a restart. Treating both as “fetch an API every so often” would hide the decisions that keep them useful.

```mermaid center
flowchart LR
  SpotifyAPI["Spotify Web API"] --> Spotify["Now-playing module"]
  GitHubAPI["GitHub GraphQL API"] --> GitHub["Contribution module"]
  Spotify --> Contract["Shared stat contracts"]
  GitHub --> Contract
  Contract --> Stream["Site telemetry stream"]
...
The [shared schema](https://github.com/ErickCReis/ErickCReis/blob/main/shared/stats/spotify.ts) is also a privacy boundary. The browser receives track metadata and a public Spotify link, not the refresh token, API credentials, raw response, device information, or playback controls.
...
The latest 84 snapshots live only in process memory. They let the panel find a previous track, but they are not listening history. Restarting the server clears them, and that is a useful distinction: the site displays playback context without building another permanent record of it.
...
This cache is not a second analytics database. It contains the same aggregate values sent to the browser and exists to avoid an empty panel and a redundant API call after a process restart. Valibot validates it before use, so an old or malformed file degrades to a fresh request instead of quietly entering the stat stream.
...
<SourcePolicyLab client:visible locale="en-US" />
...
That separation is the important part of using personal APIs as product data. The source adapter owns authentication, timing, normalization, and failure behavior. A shared contract carries only what the product surface needs. The UI renders a state rather than reenacting the external API.
