...
I designed each adapter by answering the same three questions: how quickly can this value become misleading, what may cross the browser boundary, and what should happen when the source says “not now”? The answers—not the visual size of the panel—set its policy.
...
The latest 84 snapshots live only in process memory. They let the panel find a previous track, but they are not a durable listening history. Restarting the server clears them, and that is a useful distinction: the site displays playback context without building another permanent record of it.
...
That separation is the important part of using personal APIs as product data. The source adapter owns authentication, timing, normalization, retention, and failure behavior. A shared contract carries only what the product surface needs. The UI renders a state rather than reenacting the external API.
