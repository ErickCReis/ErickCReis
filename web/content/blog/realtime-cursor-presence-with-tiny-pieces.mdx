...
Move the pointer on the homepage and a small marker follows it. Open the page in another browser and that marker becomes a remote cursor there. The visible result is playful, but its implementation is a useful example of the architecture behind the site: a few small pieces, each responsible for one edge of the interaction.
...
There are no join, leave, or room messages. This site has one public cursor surface, and a position update is all it needs. The smaller protocol also makes its limit explicit: this is transient presence, not a collaboration system.
...
Instead, the [live routes](https://github.com/ErickCReis/ErickCReis/blob/main/server/live/routes.ts) assign the WebSocket a cursor ID in an HTTP-only cookie during the upgrade. The cookie is strict same-site, secure in production, and unavailable to client-side JavaScript. A small `GET /live/id` endpoint lets the rendering layer learn its own ID without making the cookie itself readable.

Every incoming message still includes an ID, but the server only broadcasts it when that value matches the ID attached to the connection:
...
The API module maintains one WebSocket and a set of subscribers. The first subscriber connects; the last unsubscribe closes it. That matters because Astro's client-side navigation can mount and unmount the live island without a full page reload.
...
The feature works because it does not pretend to be more than it is. A server-owned identity, a validated position message, a throttled sender, a reconnecting socket, and expiring client state are enough to make the otherwise static page feel occupied.
