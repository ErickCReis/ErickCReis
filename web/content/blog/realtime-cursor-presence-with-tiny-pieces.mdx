---
title: "Real-time cursor presence with a few small pieces"
description: How pointer tracking, a typed WebSocket, cookie-backed identity, and Solid come together in one live interaction.
date: "2026-07-18"
...
The [previous post](/blog/astro-static-shell-solid-live-layer) ended at the boundary of the Solid island. This one follows a pointer across it.
...
I resisted adding join, leave, or room messages. This site has one public cursor surface, and a position update is all it needs. The smaller protocol also makes its limit explicit. Its scope stops at transient presence.
...
Positions and colors are still untrusted public input, so that check is not a complete security model. It does preserve the one property this feature relies on: one connection cannot publish as another connection's cursor.
...
The hook samples motion for the display and makes no delivery guarantee. If the socket is not open, the current update is dropped. A newer pointer position will replace it soon enough.
...
<CursorProtocolLab client:load locale="en-US" />
...
stateDiagram-v2
  ...
  Reconnecting --> Connected: retry in 1 second
  Reconnecting --> [*]: no subscribers
  Connected --> [*]: last subscriber
```
...
Those small pieces make the otherwise static page feel occupied without turning cursor positions into durable data.
